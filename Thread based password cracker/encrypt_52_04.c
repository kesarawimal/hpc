...
void *crack(arguments_t *args){
  ...
  for(x='0'; (x>='A' && x<='Z')||(x>='a' && x<='z'); (x == 'Z') ? x = 'a' : x++){
    plain[0] = x;
    for(y='0'; (y>='A' && y<='Z')||(y>='a' && y<='z'); (y == 'Z') ? y = 'a' : y++){
      plain[1] = y;
      for(z='0'; (z>='A' && z<='Z')||(z>='a' && z<='z'); (z == 'Z') ? z = 'a' : z++){
        plain[2] = z;
        for(v='0'; (v>='A' && v<='Z')||(v>='a' && v<='z'); (v == 'Z') ? v = 'a' : v++){
          plain[3] = v;
